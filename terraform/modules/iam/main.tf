...
resource "aws_secretsmanager_secret" "grafana_admin" {
  # The private deployment job creates the initial random value without placing it in Terraform state.
  #checkov:skip=CKV2_AWS_57: Rotation is managed by the deployment runbook until an automated Grafana rotation hook is introduced.
  name                    = "${var.name}/grafana-admin"
  description             = "Grafana administrator credentials generated by the private deployment job"
  recovery_window_in_days = 30
  kms_key_id              = aws_kms_key.runtime.arn
  tags                    = var.tags
}
