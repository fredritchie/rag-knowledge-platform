...
diagnose_failure() {
  local exit_code=$?
  echo "Platform installation failed; collecting Kubernetes diagnostics" >&2
  helm list --all-namespaces || true
  kubectl get nodes -o wide || true
  kubectl get deployments,statefulsets,daemonsets,pods,jobs,pvc --all-namespaces -o wide || true
  kubectl get events --all-namespaces --sort-by=.lastTimestamp | tail -200 || true
  exit "${exit_code}"
}
...
helm upgrade --install rag-platform-storage "${root_dir}/helm/storage" \
  --namespace kube-system --wait --timeout 15m
helm upgrade --install cilium cilium/cilium --version "${CILIUM_VERSION}" \
  --namespace kube-system --values "${platform_dir}/cilium-values.yaml" --wait --timeout 15m
helm upgrade --install aws-load-balancer-controller eks/aws-load-balancer-controller \
  ...
  --set clusterName="${EKS_CLUSTER_NAME}" --set region="${AWS_REGION}" --set vpcId="${VPC_ID}" \
  --wait --timeout 15m
helm upgrade --install external-secrets external-secrets/external-secrets \
  --version "${EXTERNAL_SECRETS_VERSION}" --namespace external-secrets --create-namespace \
  --values "${platform_dir}/external-secrets-values.yaml" --wait --timeout 15m
helm upgrade --install metrics-server metrics-server/metrics-server \
  --version "${METRICS_SERVER_VERSION}" --namespace kube-system \
  --values "${platform_dir}/metrics-server-values.yaml" --wait --timeout 15m
...
helm upgrade --install kube-prometheus-stack prometheus-community/kube-prometheus-stack \
  ...
  --set-string "grafana.additionalDataSources[2].jsonData.defaultRegion=${AWS_REGION}" \
  --wait --timeout 15m
helm upgrade --install loki grafana-community/loki --version "${LOKI_VERSION}" \
  ...
  --set-string loki.storage.s3.region="${AWS_REGION}" --wait --timeout 15m
...
helm upgrade --install opentelemetry-collector open-telemetry/opentelemetry-collector \
  ...
  --values "${platform_dir}/opentelemetry-collector-values.yaml" --wait --timeout 15m
helm upgrade --install alloy grafana/alloy --version "${ALLOY_VERSION}" \
  --namespace monitoring --values "${platform_dir}/alloy-values.yaml" --wait --timeout 15m
helm upgrade --install dcgm-exporter gpu-helm-charts/dcgm-exporter \
  --version "${DCGM_EXPORTER_VERSION}" --namespace monitoring \
  --values "${platform_dir}/dcgm-exporter-values.yaml" --wait --timeout 15m
helm upgrade --install rag-observability "${root_dir}/helm/observability" \
  --namespace monitoring --wait --timeout 15m
