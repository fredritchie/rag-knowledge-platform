...
export default function DriveControls({initialConnections,initialQueue}:{initialConnections:DriveConnection[],initialQueue:QueueHealth}) {
  ...
  return <div className="grid" style={{marginTop:24}}>
    ...
    <section className="card"><div className="header"><div><h2>Drive connections</h2><p>Schedule, cursor, health, errors, and lifecycle controls.</p></div><button className="secondary" onClick={refresh}>Refresh</button></div>{connections.length===0?<div className="empty">No Drive connection is configured.</div>:<table><thead><tr><th>Connection</th><th>Status</th><th>Last sync</th><th>Errors</th><th>Actions</th></tr></thead><tbody>{connections.map(item=><tr key={item.connection_id}><td><strong>{item.display_name}</strong><div className="muted">Cursor {item.last_change_token_present?"stored":"not initialized"}</div>{item.last_error&&<div style={{color:"var(--red)"}}>{item.last_error}</div>}</td><td><span className={`badge ${item.sync_status==="FAILED"?"failed":""}`}>{item.status} / {item.sync_status}</span></td><td>{item.last_success_time?new Date(item.last_success_time).toLocaleString():"Never"}<div className="muted">Next: {item.next_sync_at?new Date(item.next_sync_at).toLocaleString():"—"}</div></td><td><button className="secondary" disabled={busy} onClick={()=>viewErrors(item.connection_id)}>View {item.error_count}</button></td><td><div className="actions"><button disabled={busy} onClick={()=>action(item.connection_id,"force-sync")}>Force Sync</button>{item.status==="PAUSED"?<button className="secondary" disabled={busy} onClick={()=>action(item.connection_id,"resume")}>Resume</button>:<button className="secondary" disabled={busy} onClick={()=>action(item.connection_id,"pause")}>Pause</button>}<button className="secondary" disabled={busy} onClick={()=>action(item.connection_id,"","DELETE")}>Disconnect</button></div></td></tr>)}</tbody></table>}</section>
    {errors.length>0&&<section className="card"><h2>Synchronization errors</h2><table><thead><tr><th>Time</th><th>File</th><th>Action</th><th>Error</th></tr></thead><tbody>{errors.map(error=><tr key={error.id}><td>{new Date(error.created_at).toLocaleString()}</td><td>{error.file_id}</td><td>{error.action}</td><td>{error.error}</td></tr>)}</tbody></table></section>}
